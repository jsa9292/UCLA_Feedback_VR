...
namespace VIVE.OpenXR.Samples.Hand
{
    public class Joint_Movement : MonoBehaviour
    {
        ...
        public static Quaternion rotOffset;
        private void Start()
        {
            rotOffset = Quaternion.Euler(0, 180f, 0);
        }
        public bool showHand;
        void Update()
        {
            showHand = XR_EXT_hand_tracking.Interop.GetJointLocations(isLeft, out XrHandJointLocationEXT[] handJointLocation);
            if (!showHand) {return; }
            ...
            if (((UInt64)handJointLocation[jointNum].locationFlags & (UInt64)XrSpaceLocationFlags.XR_SPACE_LOCATION_ORIENTATION_TRACKED_BIT) != 0)
            {
                ...
                transform.rotation = jointRot;// * rotOffset;
            }
